...
func (us *unionStatementImpl) String(d Dialect) (sql string, err error) {
	if len(us.selects) == 0 {
		return "", errors.New("union statement must have at least one SELECT")
	}

	if len(us.selects) == 1 {
		return us.selects[0].String(d)
	}
	...
	for _, statement := range us.selects {
		...
		if !ok {
			return "", errors.New("expected inner select statement to be of type selectStatementImpl")
		}

		// check that for limit for statements with order by clauses
		if statementImpl.order != nil && statementImpl.limit < 0 {
			return "", errors.New("all inner selects in Union statement must have LIMIT if they have ORDER BY")
		}
	...
	for i, statement := range us.selects {
		...
		_, _ = buf.WriteRune('(')
		selectSql, err := statement.String(d)
		if err != nil {
			return "", err
		}
	...
	if us.where != nil {
		...
		if err = us.where.SerializeSql(d, buf); err != nil {
			return
		}
	}
	...
	if us.group != nil {
		...
		if err = us.group.SerializeSql(d, buf); err != nil {
			return
		}
	}
	...
	if us.order != nil {
		...
		if err = us.order.SerializeSql(d, buf); err != nil {
			return
		}
	}
...
func (q *selectStatementImpl) String(d Dialect) (sql string, err error) {
	if d.Name() != nil && !validIdentifierName(*d.Name()) {
	...
	if q.projections == nil || len(q.projections) == 0 {
		return "", errors.New("no column selected. Generated sql: " + buf.String())
	}
	...
	for i, col := range q.projections {
		...
		if col == nil {
			return "", errors.New("nil column selected. Generated sql: " + buf.String())
		}
		if err = col.SerializeSqlForColumnList(true, d, buf); err != nil {
			return
		}
	}
	...
	if q.table == nil {
		return "", errors.New("nil table. Generated sql: " + buf.String())
	}
	if err = q.table.SerializeSql(d, buf); err != nil {
		return
	}

	if q.where != nil {
		buf.WriteString(" WHERE ")
		if err = q.where.SerializeSql(d, buf); err != nil {
			return
		}
	}
	...
	if q.group != nil {
		...
		if err = q.group.SerializeSql(d, buf); err != nil {
			return
		}
	}
	...
	if q.order != nil {
		...
		if err = q.order.SerializeSql(d, buf); err != nil {
			return
		}
	}
...
func (s *insertStatementImpl) String(d Dialect) (sql string, err error) {
	if d.Name() != nil && !validIdentifierName(*d.Name()) {
		return "", errors.New("invalid database name specified")
	}
	...
	if err = s.table.SerializeSql(d, buf); err != nil {
		return
	}

	if len(s.columns) == 0 {
		return "", errors.New("no column specified.  Generated sql: " + buf.String())
	}
	...
	for i, col := range s.columns {
		...
		if col == nil {
			return "", errors.New("nil column in columns list. Generated sql: " + buf.String())
		}

		if err = col.SerializeSqlForColumnList(false, d, buf); err != nil {
			return
		}
	}

	if len(s.rows) == 0 {
		return "", errors.New("no row specified. Generated sql: " + buf.String())
	}
	...
	for row_i, row := range s.rows {
		...
		if len(row) != len(s.columns) {
			return "", errors.New("# of values does not match # of columns. Generated sql: " + buf.String())
		}
		...
		for col_i, value := range row {
			...
			if err = value.SerializeSql(d, buf); err != nil {
				return
			}
		}
		buf.WriteByte(')')
	...
	if len(s.onDuplicateKeyUpdates) > 0 {
		...
		for i, colExpr := range s.onDuplicateKeyUpdates {
			...
			if colExpr.col == nil {
				return "", errors.New("nil column in on duplicate key update list. Generated sql: " + buf.String())
			}

			if err = colExpr.col.SerializeSqlForColumnList(false, d, buf); err != nil {
				return
			}
			...
			if colExpr.expr == nil {
				return "", errors.New("nil expression in on duplicate key update list. Generated sql: " + buf.String())
			}

			if err = colExpr.expr.SerializeSql(d, buf); err != nil {
				return
			}
		}
	}
...
func (u *updateStatementImpl) String(d Dialect) (sql string, err error) {
	if d.Name() != nil && !validIdentifierName(*d.Name()) {
		return "", errors.New("invalid database name specified")
	}
	...
	if err = u.table.SerializeSql(d, buf); err != nil {
		return
	}

	if len(u.updateValues) == 0 {
		return "", errors.New("no column updated. Generated sql: " + buf.String())
	}
	...
	for col, expr := range u.updateValues {
		if col == nil {
			return "", errors.New("nil column. Generated sql: " + buf.String())
		}
	...
	for _, col := range u.table.Columns() {
		...
		if val == nil {
			return "", errors.New("nil value. Generated sql: " + buf.String())
		}
		...
		buf.WriteByte('=')
		if err = val.SerializeSql(d, buf); err != nil {
			return
		}
	...
	if u.where == nil {
		return "", errors.New("updating without a WHERE clause. Generated sql: " + buf.String())
	}

	buf.WriteString(" WHERE ")
	if err = u.where.SerializeSql(d, buf); err != nil {
		return
	}

	if u.order != nil {
		buf.WriteString(" ORDER BY ")
		if err = u.order.SerializeSql(d, buf); err != nil {
			return
		}
	}
...
func (d *deleteStatementImpl) String(dialect Dialect) (sql string, err error) {
	if dialect.Name() != nil && !validIdentifierName(*dialect.Name()) {
		return "", errors.New("invalid database name specified")
	}
	...
	if err = d.table.SerializeSql(dialect, buf); err != nil {
		return
	}

	if d.where == nil {
		return "", errors.New("Deleting without a WHERE clause. Generated sql: " + buf.String())
	}

	buf.WriteString(" WHERE ")
	if err = d.where.SerializeSql(dialect, buf); err != nil {
		return
	}

	if d.order != nil {
		buf.WriteString(" ORDER BY ")
		if err = d.order.SerializeSql(dialect, buf); err != nil {
			return
		}
	}
...
func (s *lockStatementImpl) String(d Dialect) (sql string, err error) {
	if d.Name() != nil && !validIdentifierName(*d.Name()) {
		return "", errors.New("invalid database name specified")
	}
	...
	for idx, lock := range s.locks {
		...
		if err = lock.t.SerializeSql(d, buf); err != nil {
			return
		}
...
func (s *unlockStatementImpl) String(d Dialect) (sql string, err error) {
...
func writeComment(comment string, buf *bytes.Buffer) error {
	if comment != "" {
		...
		if !isValidComment(comment) {
			return errors.New("invalid comment: " + comment)
		}
