...
// NOTE: NATURAL JOINs, and join "USING" clause
// are not supported.
...
func (t *Table) getColumn(name string) (NonAliasColumn, error) {
	...
	return nil, fmt.Errorf("no such column '%s' in table '%s'", name, t.name)
}
...
func (t *Table) SerializeSql(d Dialect, out *bytes.Buffer) error {
	...
	if t.forcedIndex != "" {
		if !validIdentifierName(t.forcedIndex) {
			return fmt.Errorf("'%s' is not a valid identifier for an index", t.forcedIndex)
		}
...
func (t *joinTable) SerializeSql(d Dialect, out *bytes.Buffer) (err error) {
	if t.lhs == nil {
		return errors.New("nil lhs. Generated sql: " + out.String())
	}
	if t.rhs == nil {
		return errors.New("nil rhs. Generated sql: " + out.String())
	}
	if t.onCondition == nil {
		return errors.New("nil onCondition. Generated sql: " + out.String())
	}
